		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					// if ($this->input->post('expiry_month')) {
					// 	$resp = $this->fetch_clover_payment_token(
					// 		$this->input->post('business_id'), $this->input->post('country_code'), $this->input->post('number'), $this->input->post('expiry_month'), $this->input->post('expiry_year'), $this->input->post('cvd'));
					//
					// 	$resp = json_decode($resp);
					// 	if ($resp->status == 0) {
					// 		$arg['status'] = 0;
					...
					if ($this->form_validation->run() == FALSE) {
						...
						//$resp = stripeCheckout()
						// $arg['status'] = 0;
						// $arg['request']  = $this->input->post();
						// $arg['$amount']  = gettype($amount);
						// $arg['userdata']  = $userdata['data']['email'];
						// $arg['userdata']  = stripeCheckout($token, floatVal($amount), $userdata['data']['email']);
						// echo json_encode($arg); exit;
