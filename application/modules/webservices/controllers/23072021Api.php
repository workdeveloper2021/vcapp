...
class Api extends REST_Controller
{
					...
					if (!empty($data)) {
						...
						if ($hashed_password == $data[0]['password']) {
							...
							if ($userdata) {
								...
								$userdata['consent_status'] = intVal($consent_step);
								$arg['status'] = 1;
								$arg['error_code'] = REST_Controller::HTTP_OK;
								$arg['error_line'] = __line__;
	...
	public function getCardData_get()
	{
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$res = strhlp_get_card_list('cus_JOIxfV6IdWmDq8');
				$arg['status'] = 1;
				$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
				$arg['error_line'] = __line__;
				$arg['data'] = $res;
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					$this->form_validation->set_rules('step', 'Step', 'required|numeric');

					if ($this->input->post('step') == 7) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$json_step_data = json_encode($this->input->post('json_step_data'));
						/*$json_step_data = json_encode($this->input->post('json_step_data'));
						print_r($json_step_data2);
						if(!is_array($json_step_data2))
						{

							$errors        = 'Please add step data json format';
							$arg['status'] = 0;
						    $arg['message']= $errors;
						 	echo json_encode($arg);
						 	die;
						...
						if ($fullfill_step == 1) {
							...
							if (!empty(@$token->id) && @$token->id != "") {
								$res = strhlp_checkout(array(
									'amount' => $amount,
									'name'	=> $name . ' ' . $lastname,
									'email'	=> $userdata['data']['email'],
									'description' => $amount . ' successfully paid!',
									'token'	=>	$token
								), array('user_id'	=>	$usid), 2);
								...
								if (strtolower($payment_status) == 'succeeded') {
									...
									$arg['error_line'] = __line__;
									$arg['data']       = array();
									$arg['message']    = 'Invalid payment details - ' . @$res['meassge'];
								}
							} else {
								$arg['status']     = 0;
								$arg['error_code'] = REST_Controller::HTTP_OK;
								$arg['error_line'] = __line__;
								$arg['data']       = array();
								$arg['message']    = 'Invalid payment details - ' . $token;
							}
						}
					}
				}
	...
	public function get_font_list_get()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			$findresult = $this->dynamic_model->get_query_result('select * from manage_font where status="Active" ');
			//var_dump($findresult); die;
			$data = array();
			$data_array = array();
			if (count($findresult) > 0) {
				foreach ($findresult as $key => $value) {
					...
					$data['id']        = $value->id;
					$data['font_name'] = $value->font_name;
	...
	public function get_newsfeed_list_get($id = '')
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$time_zone = $time_zone ? $time_zone : 'UTC';
				date_default_timezone_set($time_zone);
				$data_array_newsfeed = get_newsfeed_data($id);

				$arg['status']     = 1;
				$arg['error_code'] = REST_Controller::HTTP_OK;
				$arg['error_line'] = __line__;
				$arg['data']       = $data_array_newsfeed;
				$arg['message']    = $this->lang->line('record_found');
			}
		}
		echo json_encode($arg);
	...
	public function newsfeed_like_post()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					$this->form_validation->set_rules('user_id', 'user id', 'required');
					$this->form_validation->set_rules('newsfeed_id', 'newsfeed id', 'required');
					$this->form_validation->set_rules('is_like', 'is like', 'required|in_list[0,1]');
					if ($this->form_validation->run() == FALSE) {
						$arg['status'] = 0;
						$arg['message'] = get_form_error($this->form_validation->error_array());
					} else {
						...
						$selectData = $this->dynamic_model->get_query_result('select * from newsfeed_favourite where user_id="' . $user_id . '" and newsfeed_id="' . $newsfeed_id . '"');


						$message = ($is_like == 1) ? 'News feed like successfully' : 'News feed dislike successfully';
						$time = time();
						if (!$selectData) {
							// New record insert
							$this->dynamic_model->insertdata('newsfeed_favourite', array(
								'user_id'		=>	$user_id,
							...
							$this->dynamic_model->updateRowWhere(
								'newsfeed_favourite',
								array('user_id' => $user_id, 'newsfeed_id' => $newsfeed_id),
								array('is_like' => $is_like, 'added_date' => $time)
							);
						}
						$data_array_newsfeed = get_newsfeed_data($newsfeed_id);
						$arg['status']    = 1;
						$arg['error_code'] = REST_Controller::HTTP_OK;
						$arg['error_line'] = __line__;
						$arg['message']   = $message;
	...
	public function newsfeed_comments_post()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$newsfeed_comments_id = $this->dynamic_model->insertdata(
							'newsfeed_comments',
							array(
								'user_id' => $user_id,
								'newsfeed_id' => $newsfeed_id,
						...
						$start_limit = 0;
						$end_limit = 1;
						$newsfeed_comments_data = $this->dynamic_model->get_query_result('select * from newsfeed_comments where status="Active" and newsfeed_id="' . $newsfeed_id . '" and user_id="' . $user_id . '" order by newsfeed_comments_id desc limit ' . $start_limit . ',' . $end_limit);

						$data =  $newsfeed_comments_data[0];
						$data->added_date = date("d M-Y h:i A", $data->added_date);

						$arg['status']    = 1;
						$arg['error_code'] = REST_Controller::HTTP_OK;
						$arg['error_line'] = __line__;
						$arg['message']   = 'You have successfully added your comments';
						$arg['data'] 	  = $data;
					}
				}
			}
	...
	public function get_newsfeed_comments_list_post()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					$this->form_validation->set_rules('user_id', 'user id', 'required');
					$this->form_validation->set_rules('newsfeed_id', 'newsfeed id', 'required');
					$this->form_validation->set_rules('page_no', 'page no', 'required');
					if ($this->form_validation->run() == FALSE) {
						$arg['status'] = 0;
						$arg['message'] = get_form_error($this->form_validation->error_array());
					} else {
						...
						if ($page_no == "1") {
							...
							$start_limit = ($page_no - 1) * $end_limit;
						}

						$comments_data = array();
						$newsfeed_comments_data = $this->dynamic_model->get_query_result('select * from newsfeed_comments where status="Active" and newsfeed_id="' . $newsfeed_id . '" order by newsfeed_comments_id desc limit ' . $start_limit . ',' . $end_limit);
						if ($newsfeed_comments_data) {
							for ($i = 0; $i < count($newsfeed_comments_data); $i++) {
								$loguser = $this->dynamic_model->get_query_result('select * from user where id="' . $newsfeed_comments_data[$i]->user_id . '"');
								//var_dump($loguser); die;
								if ($loguser) {
									$username = trim($loguser[0]->name) . ' ' . trim($loguser[0]->lastname);
									$profile_img = base_url() . 'uploads/user/' . $loguser[0]->profile_img;
								} else {
									$username = '';
									$profile_img = base_url() . 'uploads/user/user.png';
								}
								$comments_data[] = array(
									'newsfeed_comments_id' => $newsfeed_comments_data[$i]->newsfeed_comments_id,
									'comments' => $newsfeed_comments_data[$i]->comments,
									'username' => trim($username),
									'img_url' => $profile_img,
									'added_date' => date("d M-Y h:i A", $newsfeed_comments_data[$i]->added_date)
								);
							}
	...
	/* Get Studio plan details */
	public function get_studio_plan_list_post()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$condition = array('studio_plan_details.business_id' => $business_id, 'studio_plan_details.status' => 'Active');
						$on = 'studio_plan_master.id = studio_plan_details.plan_id';
						$client_data = $this->dynamic_model->getTwoTableData('studio_plan_details.id, studio_plan_details.plan_id, studio_plan_master.name, studio_plan_details.price', 'studio_plan_details', 'studio_plan_master', $on, $condition);
						...
						if ($client_data) {
							...
							$arg['business_logo']      = site_url() . 'uploads/business/' . $logo;
							$arg['message']   = $this->lang->line('record_found');
						} else {
							$arg['status']     = 0;
							$arg['error_code']  = REST_Controller::HTTP_OK;
	...
	************************************************ */
	public function stripe_customer_create_post()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$_POST = json_decode(file_get_contents("php://input"), true);
				if ($_POST) {
					$this->form_validation->set_rules('token', 'Token', 'required|trim', array('required' => 'Token is required.'));
					if ($this->form_validation->run() == FALSE) {
						$arg['status'] = 0;
						$arg['message'] = get_form_error($this->form_validation->error_array());
					} else {
						...
						$userInfo = $this->dynamic_model->getdatafromtable('user', array('id' => $userid));

						if (!$userInfo) {
							$arg['status'] = 0;
							$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
							$arg['error_line'] = __line__;
							$arg['message'] = 'Invalid user details';
							$arg['data'] = json_decode('{}');
						} else {
							$customer_name = $userInfo[0];
							if (!empty($customer_name['customer_id'])) {
								$arg['status'] = 0;
								$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
								$arg['error_line'] = __line__;
								$arg['message'] = 'Customer id all ready created';
								$arg['data'] = json_decode('{}');
							} else {
								$getCustomerId = strhlp_create_customer(
									array(
										'token'	=> $token,
										'name'	=> $customer_name['name'] . ' ' . $customer_name['lastname'],
										'email'	=> $userdata['data']['email'],
										'user_id' => $userid
									)
								);
								if (!$getCustomerId) {
									...
									$arg['error_code'] = HTTP_OK;
									$arg['error_line'] = __line__;
									$arg['message']   = "Customer Id Created Successfully";
									$arg['data'] = json_decode('{}');
								}
							}
						}
	...
	/* ************************************************
	Customer Purchase studio plan
	************************************************ */
	public function studio_plan_buy_post()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$_POST = json_decode(file_get_contents("php://input"), true);
				if ($_POST) {
					$this->form_validation->set_rules('business_id', 'Business Id', 'required|trim|numeric', array('required' => 'Business id is required'));
					$this->form_validation->set_rules('studio_plan_id', 'Studio Plan Id', 'required|trim|numeric', array('required' => 'Studio plan id is required.'));
					$this->form_validation->set_rules('token', 'Token', 'required|trim', array('required' => 'Token is required.'));
					if ($this->form_validation->run() == FALSE) {
						$arg['status'] = 0;
						$arg['message'] = get_form_error($this->form_validation->error_array());
						...
						$userid = $userdata['data']['id'];
						$token = $this->input->post('token');
						$plan_id = $this->input->post('studio_plan_id');

						$checkPlan = $this->dynamic_model->getdatafromtable('studio_plan_details', array('business_id' => $business_id, 'plan_id' => $plan_id, 'status' => 'Active'));

						if (!$checkPlan) {
							$arg['status'] = 0;
							$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
							$arg['error_line'] = __line__;
							$arg['message'] = 'Invalid plan details';
							$arg['data'] = json_decode('{}');
						} else {

							$start_date = date('Y-m-d');
							$end_date = date("Y-m-d", strtotime("+ 28 day"));
							...
							if (!$userInfo) {
								...
								$customer_name = $userInfo[0]['name'];
								if (!empty($userInfo[0]['lastname'])) {
									$customer_name .= " " . $userInfo[0]['lastname'];
								}
								$customer_name = trim($customer_name);
								$customer_email = $userInfo[0]['email'];
								$purchase_amount = $checkPlan[0]['price'];

								$stripe_data = array(
									'amount'	=>	$purchase_amount,
									'name'		=>	$customer_name,
									'email'		=>	$customer_email,
									'description' => SITE_NAME . ' , Studio plan purchase',
									'token'	=>	$token
								);
								...
								if ($res['status'] == 0) {
									...
									$this->dynamic_model->insertdata('studio_plan_subscription', $insertStudioPlan);
									$arg['status'] = 1;
									$arg['error_code'] = HTTP_OK;
									$arg['error_line'] = __line__;
									$arg['message']   = "Plan purchase successfully";
								}
							}
						}
					}
