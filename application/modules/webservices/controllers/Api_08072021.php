			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						if ($selectData) {
							if ($is_like == 1) //for like
							{
								$arg['status']    = 1;
								$arg['error_code'] = REST_Controller::HTTP_OK;
								$arg['error_line'] = __line__;
								$arg['message']   = 'You have already added in favourite list.';
								$arg['data'] 	  = array();
							} else {
								//$this->dynamic_model->deletedata('newsfeed_favourite', array('user_id' => $user_id, 'newsfeed_id' => $newsfeed_id));

								$newsfeed_favourite_update = $this->dynamic_model->updateRowWhere('newsfeed_favourite', array('user_id' => $user_id, 'newsfeed_id' => $newsfeed_id, 'is_like' => 1), array('added_date' => date('Y-m-d H:i:s'), 'is_like' => 0));

								$arg['status']    = 1;
								$arg['error_code'] = REST_Controller::HTTP_OK;
								$arg['error_line'] = __line__;
								$arg['message']   = 'You have successfully removed form favourite list.';
								$arg['data'] 	  = array();
							}
						} else {
							if ($is_like == 1) //for like
							{
								$newsfeed_favourite_id = $this->dynamic_model->insertdata('newsfeed_favourite', array('user_id' => $user_id, 'newsfeed_id' => $newsfeed_id, 'is_like' => $is_like, 'added_date' => date('Y-m-d H:i:s')));

								$arg['status']    = 1;
								$arg['error_code'] = REST_Controller::HTTP_OK;
								$arg['error_line'] = __line__;
								$arg['message']   = 'You have successfully added in favourite list';
								$arg['data'] 	  = array();
							} else {
								$arg['status']    = 0;
								$arg['error_code'] = REST_Controller::HTTP_OK;
								$arg['error_line'] = __line__;
								$arg['message']   = 'Record not found in your favourite list';
								$arg['data'] 	  = array();
							}
						}
					}
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$comments = $this->input->post('comments');
						$time = time();
						$newsfeed_comments_id = $this->dynamic_model->insertdata('newsfeed_comments', array('user_id' => $user_id, 'newsfeed_id' => $newsfeed_id, 'comments' => $comments, 'added_date' => $time));
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$comments_data = array();
						$newsfeed_comments_data = $this->dynamic_model->get_query_result('select * from newsfeed_comments where status="Active" and newsfeed_id="' . $newsfeed_id . '" and user_id="' . $user_id . '" order by newsfeed_comments_id asc limit ' . $start_limit . ',' . $end_limit);
