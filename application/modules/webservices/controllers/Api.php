...
class Api extends REST_Controller
{
	...
	public function testt_get(){
		...
		$descrpt = '<p>Hi {USERNAME},</p> <p>Thanks for registering on {SITE_TITLE}.</p><p> Please verify your account by click on the Link : {URL}</p>';
		$desc = str_replace('{USERNAME}', 'Ritesh', $descrpt);
		...
		if ($_POST) {
			...
			if ($version_result['status'] != 1) {
				...
				$this->form_validation->set_rules('email', 'Email', 'required|valid_email', array(
					'required' => $this->lang->line('email_required'), 'valid_email' => $this->lang->line('email_valid'),
				));
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$role = $this->input->post('role')?$this->input->post('role'):3;
					$profession = $this->input->post('profession')?$this->input->post('profession'):'';
					...
					$result = $this->dynamic_model->check_user_role($email, $role, 0);
					//print_r($result);die;
					...
					if (!empty($result)) {
						...
						if (!empty($_FILES['profile_image']['name'])) {
							$image = $this->dynamic_model->fileupload('profile_image', 'uploads/user','Picture');
						}

						// https://virtualcatalogue.consagous.co.in/admin/uploads/user/1627384606paly_video.jpeg
						...
						$where = array('device_token' => $device_token, 'device_type' => $device_type);
						$result = $this->dynamic_model->check_skip_register_user_role($device_token, 3);
						//print_r($result);die;
						...
						if (!empty($result)) {
								...
								$newuserid = $findresult[0]['id'];

								$where = array('id' => $newuserid);

								$userdata = array('name' => $name, 'role_id' => 3, 'email_otp' => $uniquemobile, 'password' => $hashed_password, 'email' => $email, 'profession' => $profession, 'profile_img' => $image, 'status' => 'Deactive', 'email_verified' => $email_verified, 'create_dt' => $time, 'update_dt' => $time);

							
								$this->dynamic_model->updateRowWhere('user', $where, $userdata);
							...
							// $userdata = array('name' => $name, 'email_otp' => $uniquemobile, 'password' => $hashed_password, 'email' => $email, 'profile_img' => $image, 'status' => 'Active', 'email_verified' => 1, 'create_dt' => $time, 'update_dt' => $time);
						...
						if ($newuserid) {
							...
							$link = 'Verification Code: ' . $uniquemobile;

							$where1 = array('slug' => 'sucessfully_registration');
							$template_data = $this->dynamic_model->getdatafromtable('manage_notification_mail', $where1);
							$descrpt = '<p>Hi {USERNAME},</p> <p>Thanks for registering on {SITE_TITLE}.</p><p> Please verify your account by click on the Link : {URL}</p>';
							$desc = str_replace('{USERNAME}', $name, $descrpt);
							$desc_data = str_replace('{URL}', $url, $desc);
							$desc_send = str_replace('{SITE_TITLE}', SITE_TITLE, $desc_data);
							$subject = str_replace('{SITE_TITLE}', SITE_TITLE, $template_data[0]['subject']);
							$emailsubject = 'Thank you for registering with ' . SITE_TITLE;
...
public function skip_registration_post()
	{
		$_POST = json_decode(file_get_contents("php://input"), true);
		$arg = array();
		if ($_POST) {
			...
			if ($version_result['status'] != 1) {
				...
				$this->form_validation->set_rules('device_type', 'Device Type', 'required', array(
					'required' => $this->lang->line('device_type_required'),
				));
				
				$this->form_validation->set_rules('device_token', 'Device Token', 'required', array(
					'required' => $this->lang->line('device_token_required'),
				));
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$where = array('device_token' => $device_token, 'device_type' => $device_type);
					$result = $this->dynamic_model->check_skip_register_user_role($device_token, $role);
					//print_r($result);die;
					...
					if (!empty($result)) {
							...
							$tokendata = array('userid'=>$id,'token'=>$token,'skipped_registration'=>true);
							...
							$arg['error_code'] = REST_Controller::HTTP_OK;
							$arg['error_line'] = __line__;
							$arg['message'] = "Skipped registration successfully";
							$arg['data'] = array("authorization"=>$user_token);
						...
						$userdata = array('role_id' => 3, 'status' => 'Deactive', 'email_verified' => $email_verified, 'create_dt' => $time, 'update_dt' => $time, 'device_token' => $device_token, 'device_type' => $device_type);
						...
						if ($newuserid) {
								...
								$tokendata = array('userid'=>$newuserid,'token'=>$token,'skipped_registration'=>true);
								$user_token = base64_encode(json_encode($tokendata));
								...
								$arg['error_code'] = REST_Controller::HTTP_OK;
								$arg['error_line'] = __line__;
								$arg['message'] ="Logged in as a guest successfully.";
								$arg['data'] = array("authorization"=>$user_token);
	...
	public function verify_otp_post()
	{
		...
		if ($_POST) {
			$arg = array();
			$this->form_validation->set_rules('identity', 'User Identitiy', 'required');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($result) {
					$id = $result[0]['id'];
					$mobile_verified = $result[0]['email_verified'];
					$mobile_otp_date = $result[0]['mobile_otp_date'];
					$mobile_otp = $result[0]['email_otp'];
					...
					if ($email_verified == '1') {
						...
						$update_data = array('email_verified' => "1", 'status' => "Active");
					}
	...
	public function reset_password_post()
	{
		...
		if ($_POST) {
			$arg = array();
			$this->form_validation->set_rules('identity', 'User Identity', 'required');
			$this->form_validation->set_rules('otp', 'OTP', 'required|max_length[6]');
			...
			if ($this->form_validation->run() == FALSE) {
				...
				if ($result) {
					...
					$update_data = array('password' => encrypt_password($this->input->post('password')));
	...
	public function login_post()
	{
		...
		if ($_POST) {
			...
			if ($version_result['status'] != 1) {
				...
				if ((!empty($_POST['device_type'])) && $_POST['device_type'] !== 'Browser') {
					$this->form_validation->set_rules('device_token', 'Device Token', 'required', array(
						'required' => $this->lang->line('device_token_required'),
					));
				}
				...
				// 		'required' => $this->lang->line('role_required'),
				// 	));
				// }
				if ($this->form_validation->run() == FALSE) {
					$arg['status'] = 0;
					...
					$time = time();
					$where = array('email' => $email);
					$data = $this->dynamic_model->check_user_role($email, $role_id, 0, null);
						...
						if ($hashed_password == $data[0]['password']) {
							...
							if ($userdata) { 
								if ($userdata['email_verified'] != 1) {
									...
									$tokenupdate = array('create_dt' => $time, "email_otp"=>$uniquemobile);
									...
									// $where1 = array('slug' => 'sucessfully_registration');
									// $template_data = $this->dynamic_model->getdatafromtable('manage_notification_mail', $where1);
									// $descrpt = '<p>Hi {USERNAME},</p> <p>Thanks for registering on {SITE_TITLE}.</p><p> Please verify your account by {URL}</p>';
									// $desc = str_replace('{USERNAME}', $name, $descrpt);
								...
								// $user_token = base64_encode(json_encode($tokendata));
								// $decodeVal = json_decode(base64_decode($user_token));
								...
								$tokendata = array('userid'=>$userid,'token'=>$token);
								$user_token = base64_encode(json_encode($tokendata));
								// $find_result = $this->dynamic_model->get_query_result('select * from user_registration where status="Active" and user_id="' . $userid . '" ');
								// $consent_step = 0;
								// if ($find_result) {
								// 	$info = $find_result[0];
								// 	$consent_step = $info->fullfill_step;
								// } 
								// $consent_url = str_replace('superadmin/', 'consent-form?id=', base_url());
								// $call_back = str_replace('superadmin/', 'home', base_url());
								// $userdata['consent_url'] = $consent_url;
								// $userdata['consent_callback_url'] = $call_back;
								$userdata['auth_token'] = $user_token;
								$arg['status'] = 1;
								$arg['error_code'] = REST_Controller::HTTP_OK;
								$arg['error_line'] = __line__;
		...
		if ($_POST) {
			...
			if ($version_result['status'] != 1) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if ($forcheck) {
						// $role2 = ($role_id == 3) ? 4 : 3;
						$roleExist = $this->dynamic_model->check_user_role($email, $role_id, 0, null);
						if (empty($roleExist)) {
							$arg['status'] = 0;
							$arg['error_code'] = 0;
							$arg['error_line'] = __line__;
							...
							$update_data = array("email_otp"=>$uniquemobile);
							...
							$desc_data = str_replace('{OTP}', $uniquemobile, $desc);
							$desc_send = str_replace('{SITE_TITLE}', SITE_TITLE, $desc_data);
							$subject = str_replace('{SITE_TITLE}', SITE_TITLE, $template_data[0]['subject']);
							...
							if ($mailsent == 1) {
								$update_data = array('password' => encrypt_password($otpnumber), "email_otp"=>$uniquemobile);
								$wheres = array("id" => $userid);
								$updatedata = $this->dynamic_model->updateRowWhere("user", $wheres, $update_data);

								$data_val = array('identity' => encode($userid));
								$arg['status'] = 1;
								$arg['error_code'] = REST_Controller::HTTP_OK;
								$arg['error_line'] = __line__;
								$arg['data'] = $data_val;
								$arg['message'] = "OTP sent successfully to your registered email to reset your password";
							} else {
								$arg['status'] = 0;
	...
	public function home_page_get()
	{
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] == 1) {
				...
				if($userdata['data']['skipped_registration']){

					$info = array(
						"name" => "Guest",
						"profile_img" => base_url().'uploads/user/userdefault.png',
						"options" => $options
					);
	...
	public function get_products_list_get()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] == 1) {
						...
						$data = $this->api_model->get_product_list();
						...
						if(!$data){
							...
							$arg['error_line'] = __line__;
							$arg['data'] = $data;
							$arg['message'] = "Products listed successfully.";
						}
	...
	public function get_company_locations_list_get()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] == 1) {
						...
						$data = $this->api_model->get_location_list();
						...
						if(!$data){
							...
							$arg['error_line'] = __line__;
							$arg['data'] = $data;
							$arg['message'] = "Location listed successfully.";
						}
	...
	public function get_companies_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							$this->form_validation->set_rules('page', 'Page No.', 'required|numeric');
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$location = $this->input->post('location_id')?$this->input->post('location_id'):'';
								$product = $this->input->post('product_id')?$this->input->post('product_id'):'';
								$page = (int)$this->input->post('page');
								$page = $page>0?$page:1;
								$per_page = COMPANY_PER_PAGE;
								$offset = $per_page*($page-1);

								// $data = $this->api_model->get_companies($location,$product,$per_page,$offset);
								$data = $this->api_model->get_companies($location,$product);
								...
								if($data && count($data)>0){
									...
									foreach ($data as $key => $value) {
										...
										$thumbPath = base_url() . 'uploads/company_media/'.$value["thumbnail"];
										$array[$key]["thumbnail"] = $thumbPath;
										$videoPath = base_url() . 'uploads/company_media/'.$value["video_url"];
										$array[$key]["video_url"] = $videoPath;
										$array[$key]["company_location"] = $value["company_location"]; 
										if((int)$value["types_count"]>1){
											$array[$key]["types_company"] = "View all";
										}else{
											$array[$key]["types_company"] = $value["type_name"];
										}

										$array[$key]["info"] = $value["info"];
									}
								...
								if(count($array)==0){
									...
									$arg['error_line'] = __line__;
									$arg['data'] = $array;
									$arg['message'] = "Companies listed successfully.";
								}
							}
						...
						$arg = $userdata;
									...
									$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
									$arg['error_line'] = __line__;
									$arg['message'] = "Body parameteres are required";
									$arg['data'] = array();
	...
	public function get_company_product_categories_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							$this->form_validation->set_rules('page', 'Page No.', 'required|numeric');
							$this->form_validation->set_rules('company_id', 'Company Id', 'required|numeric');
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$page = (int)$this->input->post('page');
								$page = $page>0?$page:1;
								$per_page = PRODUCT_CATEGORIES_PER_PAGE;
								$offset = $per_page*($page-1);

								$data = $this->api_model->get_company_product_categories($company_id,$per_page,$offset);
								...
								if(!$data){
									...
									$data11 = $this->dynamic_model->getdatafromtable('manage_company_list', $where1);

									$thumbnail = $data11[0]['thumbnail']?$data11[0]['thumbnail']:'userdefault.png';

									$arg['status'] = 1;
									$arg['error_code'] = REST_Controller::HTTP_OK;
									$arg['error_line'] = __line__;
									$arg['data'] = $data;
									$arg['thumbnail'] = base_url().'uploads/company_media/'.$thumbnail;
									$arg['message'] = "Company's product categories listed successfully.";
								}
							}
	...
	public function get_overall_product_categories_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$data = $this->api_model->get_overall_product_categories($per_page,$offset);
								...
								if(!$data){
									...
									$arg['data'] = $data;
									// $arg['thumbnail'] = base_url().'uploads/company_media/'.$thumbnail;
									$arg['message'] = "Product categories listed successfully.";
								}
							}
	...
	public function get_all_product_categories_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$data = $this->api_model->get_all_product_categories_post($per_page,$offset);
								...
								if(!$data){
									...
									$arg['error_line'] = __line__;
									$arg['data'] = $data;
									$arg['message'] = "All product categories listed successfully.";
								}
							}
	...
	public function get_company_products_post() 
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								// $location = $this->input->post('location_id')?$this->input->post('location_id'):'';
								$company_id = $this->input->post('company_id')?$this->input->post('company_id'):'';
								$category = $this->input->post('category_id')?$this->input->post('category_id'):'';
								$page = (int)$this->input->post('page');
								$page = $page>0?$page:1;
								$per_page = PRODUCT_PER_PAGE;
								$offset = $per_page*($page-1);

								$data = $this->api_model->get_company_products($company_id,$category,$per_page,$offset,$userdata['data']['id']);
								...
								if(!$data){
									...
									foreach($data as $key =>  $val){
										...
										$data1 = $this->api_model->get_company_products_color($val["product_list_id"]);
										if(!$data1){
											$data[$key]["colours"] = array();
										}else{
											$data[$key]["colours"] = $data1;


											foreach ($data1 as $key2 => $value2) {

												$data2 = $this->api_model->get_products_image($val["product_list_id"],$value2["id"]);
												if(!$data2){
													$data[$key]["colours"][$key2]["product_thumbnail"] = array();
												}else{
													$data[$key]["colours"][$key2]["product_thumbnail"] = $data2[0];
												}
										...
										$data1 = $this->api_model->get_company_products_image($val["product_list_id"]);
										if(!$data1){
											$data[$key]["product_thumbnail"] = base_url().'uploads/company_media/'.'userdefault.png';
										}else{
											$data[$key]["product_thumbnail"] = base_url().'uploads/company_media/'.$data1[0]["media"];
										}
									}
									...
									$arg['error_line'] = __line__;
									$arg['data'] = $data;
									$arg['message'] = "Company's products listed successfully.";
								}
							}
		...
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							// $this->form_validation->set_rules('page', 'Page No.', 'required|numeric');
							$this->form_validation->set_rules('product_list_id', 'Product List Id', 'required|numeric');
							// $this->form_validation->set_rules('category_id', 'Category Id', 'required|numeric');
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$product_list_id = $this->input->post('product_list_id')?$this->input->post('product_list_id'):'';

								$data = $this->api_model->get_product_details($product_list_id,$userdata['data']['id']); 
								...
								if(!$data){
									...
									$where1 = array('user_id' => $userdata['data']['id'], 'product_list_id'=> $product_list_id);
									$data11 = $this->dynamic_model->getdatafromtable('product_enter_count', $where1);


									if(!empty($data11) && count($data11)>0){
									}else{

										$userdata11 = array('user_id' => $userdata['data']['id'], 'product_list_id' => $product_list_id, 'created_on' => time());

										$this->dynamic_model->insertdata('product_enter_count', $userdata11);
									}
									...
									foreach($data as $key =>  $val){
										...
										$data[$key]["details"] = array(
																		array("details"=>$data[$key]["details1"]),
																		array("details"=>$data[$key]["details2"]),
																		array("details"=>$data[$key]["details3"])			
																	   );
										...
										if(!$data1){
											...
											foreach ($data1 as $key2 => $value2) {
												...
												$data2 = $this->api_model->get_products_image($val["product_list_id"],$value2["id"]);
												if(!$data2){
													$data[$key]["colours"][$key2]["product_images"] = array();
												}else{
													$data[$key]["colours"][$key2]["product_images"] = $data2;
												}
										...
										$data1 = $this->api_model->product_3d_rendered_images($val["product_list_id"]);
										if(!$data1){
											$data[$key]["product_3d_rendered_images"] = array();
										}else{
											$data[$key]["product_3d_rendered_images"] = $data1;
										}


										$whereIs = array('id' => $val["product_list_id"]);
										$findresult = $this->dynamic_model->getdatafromtable('product_list', $whereIs);
										$data2 = $this->api_model->product_same_category($findresult[0]['company_id'],$findresult[0]['category_id'],$val["product_list_id"],$userdata['data']['id']);
										// $data3 = $this->api_model->get_products_image($val["product_list_id"]);
										...
										if(!$data2){
											$data[$key]["product_same_category"] = array();
										}else{


											foreach($data2 as $key2 =>  $val2){

												$data3 = $this->api_model->get_company_products_image($val2["product_list_id"]);
												if(!$data3){
													$data2[$key2]["product_thumbnail"] = base_url().'uploads/company_media/'.'userdefault.png';
												}else{
													$data2[$key2]["product_thumbnail"] = base_url().'uploads/company_media/'.$data3[0]["media"];
												}
											}



											$data[$key]["product_same_category"] = $data2;
										}
									...
									$arg['error_line'] = __line__;
									$arg['data'] = $data[0];
									$arg['message'] = "Product details listed successfully.";
								}
							}
	...
	public function product_render_icon_taps_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$arg['message'] = get_form_error($this->form_validation->error_array());
									...
									$where1 = array('user_id' => $userdata['data']['id'], 'product_list_id'=> $product_list_id);
									$data = $this->dynamic_model->getdatafromtable('product_render_icon_taps_count', $where1);
									...
									if(!empty($data) && count($data)>0){
											...
											$arg['error_line'] = __line__;
											$arg['data'] = array();
											$arg['message'] = "Already visited.";

									}else{

										$userdata = array('user_id' => $userdata['data']['id'], 'product_list_id' => $product_list_id, 'created_on' => time());

										$product_render_icon_taps_count = $this->dynamic_model->insertdata('product_render_icon_taps_count', $userdata);


										if(!$product_render_icon_taps_count){
											$arg['status'] = 0;
											$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
											$arg['error_line'] = __line__;
											...
											$arg['error_line'] = __line__;
											$arg['data'] = array();
											$arg['message'] = "Visit counted successfully.";
										}
	...
	public function three_d_modal_count_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
					...
					if ($userdata['status'] == 1) { 
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$arg['message'] = get_form_error($this->form_validation->error_array());
									...
									$where1 = array('user_id' => $userdata['data']['id'], 'product_list_id'=> $product_list_id);
									$data = $this->dynamic_model->getdatafromtable('three_d_modal_count', $where1);
									...
									if(!empty($data) && count($data)>0){
										...
										$three_d_modal_count = $this->dynamic_model->insertdata('three_d_modal_count', $userdata);


										if(!$three_d_modal_count){
											$arg['status'] = 0;
											$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
											$arg['error_line'] = __line__;
											$arg['message'] = $this->lang->line('server_problem');
	...
	public function make_product_favourite_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							$this->form_validation->set_rules('product_list_id', 'Product List Id', 'required|numeric');
							$this->form_validation->set_rules('favourite_status', 'Favourite Status', 'required|numeric');
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$favourite_status = $this->input->post('favourite_status');
								$user_id = $userdata['data']['id'];
								$data = $this->api_model->make_product_favourite($product_list_id,$favourite_status,$user_id);

								if(!$data){
									$arg['status'] = 0;
									$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
									$arg['error_line'] = __line__;
									$arg['message'] = 'Unable to change favourite status';
									$arg['data'] = array();
								}else{
									$arg['status'] = 1;
									$arg['error_code'] = REST_Controller::HTTP_OK;
									$arg['error_line'] = __line__;
									$arg['data'] = $data;
									$arg['message'] = "Favourite status updated successfully.";
								}
							}
	...
	public function get_showrooms_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$page = $page>0?$page:1;
								$per_page = SHOWROOM_PER_PAGE;
								$offset = $per_page*($page-1);

								$data = $this->api_model->get_showrooms($company_id,$per_page,$offset);
								...
								if(!$data){
									...
									foreach ($data as $key => $value) {
										$cordinates = $this->api_model->get_showrooms_cordinates($value['id']);
										$array = array();
										if(!empty($cordinates)){

											foreach ($cordinates as $key2 => $eachCordinate) {
												array_push($array, array(
													"id"=>$eachCordinate['id'],
													"co_ordinates" => array($eachCordinate['xval'],$eachCordinate['yval'],$eachCordinate['zval']),
													'info'=>$eachCordinate['info']
												));
											}
											$data[$key]['co_ordinates']=$array;
		...
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
				$this->form_validation->set_rules('email', 'Email', 'required|valid_email|is_unique[user.email]', array(
					'required' => $this->lang->line('email_required'), 'valid_email' => $this->lang->line('email_valid'), 'is_unique' => $this->lang->line('email_unique'),
				));
	...
	public function plateform_chareges_get()
	{
		$condition = array('status' => 'Active');
		$setting = $this->dynamic_model->getdatafromtable("manage_setting", $condition, 'id, name, value');
		$arg['status']     = 1;
			...
			if ($userdata['status'] == 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						if ($business) {
							$isPlanPurchase = $this->dynamic_model->getdatafromtable(
								'studio_plan_subscription', 
								array(
							...
							$business['isPlanPurchase'] = false;
							$business['plan_start_date'] = '';
							$business['plan_end_date'] = '';
							if ($isPlanPurchase) {
								$planInfo = $isPlanPurchase[0];
								$business['isPlanPurchase'] = true;
								$business['plan_start_date'] = $planInfo['start_date'];
								$business['plan_end_date'] = $planInfo['end_date'];
							}
						...
						if ($attendance_for == '0' || $attendance_for == '1') {
							...
							if (!empty($class_data)) {
								foreach ($class_data as $value) {
									...
									$classesdata['status'] = ($status == 'absence') ? 'Absent' : ($status == 'cancel' ? 'Cancelled' : ($status == 'waiting' ? 'Waiting' : ($status == 'checkin' ? 'Checked-In' : ($status == 'singup' ? 'signed-up' : 'Checked-Out'))));
									$classesdata['class_type'] = get_categories($value['class_type']);
									$instructor_data = $this->instructor_list_details($business_id, 1, $value['id']);
									$classesdata['instructor_details'] = $instructor_data;
									$classesdata['create_dt'] = date("d M Y ", $value['create_dt']);
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$service_data = $this->dynamic_model->getdatafromtable(
							'service',
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$updateStatus = $this->dynamic_model->updateRowWhere('user_card_save', array('id' => $card_id, 'profile_id' => $profile_id, 'user_id' => $userid), array('id_deleted' => 1));
	...
	public function cardDelete_bk_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						//print_r($res);
						$payment_status = $res['response']->status;
						$payment_id = $res['response']->balance_transaction;
						...
						if (strtolower($payment_status) == 'succeeded') {
							...
							$ref_num    = getuniquenumber();
							$payment_id = $payment_id; //time(); // !empty($res->id) ? $res->id : $ref_num;
							$authorizing_merchant_id = time(); //$res->source->id;
							$payment_type   = 'Card';
							$payment_method = 'Online';
							$amount         = $amount;
	...
	//Used function for clover buy now new
	public function clover_buy_now_single_07052021_post()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
	...
	Stripe Token Create
	************************************************ */
	public function get_test_stripe_token_get()
	{
		$token = strhlp_get_token(
			array(
	...
	public function get_all_products_postOld() 
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								// $location = $this->input->post('location_id')?$this->input->post('location_id'):'';
								$filterArr = array();
								$filterArr['company_id'] = $this->input->post('company_id')?$this->input->post('company_id'):'';
								$filterArr['category_id'] = $this->input->post('category_id')?$this->input->post('category_id'):'';
								$filterArr['product_name'] = $this->input->post('product_name')?$this->input->post('product_name'):'';
								$page = (int)$this->input->post('page');
								$page = $page>0?$page:1;
								$per_page = PRODUCT_PER_PAGE;
								$offset = $per_page*($page-1);

								$data = $this->api_model->get_all_products($filterArr,$per_page,$offset,$userdata['data']['id']);
								...
								if(!$data){
									...
									foreach($data as $key =>  $val){
										...
										if(!$data1){
											...
											foreach ($data1 as $key2 => $value2) {
												...
												if(!$data2){
													...
													$data[$key]["colours"][$key2]["product_thumbnail"] = $data2[0]["product_image"];
												}
												...
												if($this->input->post('device_type')=='android'){
													...
													foreach ($data1 as $key2 => $value2) {
														...
														$data2 = $this->api_model->products_3d_model_color($val["product_list_id"],$value2["id"],'glb');
														if(!$data2){
															$data[$key]["colours"][$key2]["products_3d_model"] = '';
															$data[$key]["colours"][$key2]["products_3d_model_id"] = '';
														}else{
															$data[$key]["colours"][$key2]["products_3d_model"] = $data2[0]["products_3d_model"];
															$data[$key]["colours"][$key2]["products_3d_model_id"] = $data2[0]["id"];
														}
												...
												if($this->input->post('device_type')=='ios'){

													foreach ($data1 as $key2 => $value2) {

														$data2 = $this->api_model->products_3d_model_color($val["product_list_id"],$value2["id"],'usdz');
														if(!$data2){
															$data[$key]["colours"][$key2]["products_3d_model"] = '';
															$data[$key]["colours"][$key2]["products_3d_model_id"] = '';
														}else{
										...
										// $data1 = $this->api_model->get_company_products_image360($val["product_list_id"]);
										// if(!$data1){
										// 	$data[$key]["product_image_360"] = base_url().'uploads/company_media/'.'userdefault.png';
										// }else{
										// 	$data[$key]["product_image_360"] = base_url().'uploads/company_media/'.$data1[0]["media"];
										// }


										if($this->input->post('device_type')=='android'){

											$data1 = $this->api_model->products_3d_model($val["product_list_id"],'glb');
											if(!$data1){
												$data[$key]["products_3d_model"] = array();
											}else{
												$data[$key]["products_3d_model"] = $data1[0];
											}

										}else if($this->input->post('device_type')=='ios'){

											$data1 = $this->api_model->products_3d_model($val["product_list_id"],'usdz');
											if(!$data1){
												$data[$key]["products_3d_model"] = array();
											}else{
												$data[$key]["products_3d_model"] = $data1[0];
	...
	public function get_favourite_products_post() 
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$data = $this->api_model->get_favourite_products_companies($category,$per_page,$offset,$userdata['data']['id']);
								...
								if(!$data){
									...
									foreach ($data as $key => $value) {
										...
										$data2 = $this->api_model->get_company_favourite_products($cid,$category,$per_page,$offset,$userdata['data']['id']);
											...
											if(!$data2){
												...
												foreach($data2 as $key2 =>  $val){
													...
													if(!$data1){
														...
														$data2[$key2]["product_thumbnail"] = base_url().'uploads/company_media/'.$data1[0]["media"];
													}
												}
										...
										$data[$key]['favourite_products'] = $data2;


										// Fav. Products Ends


										// Country city name group

										$data2 = $this->api_model->get_companies_retailers_countries($cid);
											...
											if(!$data2){
												...
												foreach($data2 as $key2 => $val){
													...
													$data3 = $this->api_model->get_companies_retailers_cities($cid,$val["country_id"]);
													...
													if(!$data3){
														...
														foreach ($data3 as $key3 => $val3) {
															$data4 = $this->api_model->get_companies_retailers_users($cid,$val3["city"]);
															$data3[$key3]['retailers'] = $data4;
														}

													}

													$data2[$key2]['cities'] = $data3;
												}
										...
										$data[$key]['retailers_data'] = $data2;

										// Country city name group Ends
									...
									$arg['error_line'] = __line__;
									$arg['data'] = $data;
									$arg['message'] = "Favourite products with companies & retailers listed successfully.";
								}
							}
	...
	public function mail_retailer_post_old()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true); 
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1 && (!$userdata['data']['skipped_registration'])) {


							$this->form_validation->set_rules('retailer_id', 'Retailer Id', 'required|numeric');
							$this->form_validation->set_rules('company_id', 'Company Id', 'required|numeric');
							...
							if ($this->form_validation->run() == FALSE) {
								...
								// $location = $this->input->post('location_id')?$this->input->post('location_id'):'';
								$company_id 	= $this->input->post('company_id')?$this->input->post('company_id'):'';
								$retailer_id 	= $this->input->post('retailer_id')?$this->input->post('retailer_id'):'';
								$userId 		= $userdata['data']['id'];
								// $email 			= "kirtisagar.prajapat@consagous.co";
								// $name           = "kirtisagar";

								$where1 = array('id' => $retailer_id);
								$retialer_data = $this->dynamic_model->getdatafromtable('company_retailers', $where1);
								$name = $retialer_data[0]["name"];
								$email = $retialer_data[0]["email"];


								$where1 = array('slug' => 'alert_retailer_by_user');
								$template_data = $this->dynamic_model->getdatafromtable('manage_notification_mail', $where1);
								$descrpt = '<p>Hi {USERNAME},</p> <p>Thanks for registering on {SITE_TITLE}.</p><p> Please verify your account by click on the Link : {URL}</p>';
								$descrpt = $template_data[0]['description'];
								$desc_data = str_replace('{USERNAME}', $name, $descrpt);
								// $desc_data = str_replace('{URL}', $url, $desc);
								$desc_send = str_replace('{SITE_TITLE}', SITE_TITLE, $desc_data);
								// $subject = str_replace('{SITE_TITLE}', SITE_TITLE, $desc_data);
								$subject = str_replace('{SITE_TITLE}', SITE_TITLE, $template_data[0]['subject']);
								$emailsubject = str_replace('{SITE_TITLE}', SITE_TITLE, $template_data[0]['subject']);
								$data['subject'] = $subject;
								$data['description'] = $desc_send;
								$data['body'] = "";
								$msg = $this->load->view('emailtemplate', $data, true);
								...
								// $desc = str_replace('{USERNAME}', $name, $descrpt);
								// // $desc_data = str_replace('{URL}', $url, $desc);
								// $desc_send = str_replace('{SITE_TITLE}', SITE_TITLE, $desc);
								// $subject = str_replace('{SITE_TITLE}', SITE_TITLE, $template_data[0]['subject']);
								// $emailsubject = 'Thank you for registering with ' . SITE_TITLE;
								// $data['subject'] = $subject;
								// $data['description'] = $desc_send;
								...
								$userdata = array('user_id' => $userId, 'retailer_id' => $retailer_id, 'company_id' => $company_id, 'created_on' => time());

								$retailer_mail_count = $this->dynamic_model->insertdata('retailer_mail_count', $userdata);
								...
								if(!$data){
									...
									$arg['error_line'] = __line__;
									$arg['data'] = array();
									$arg['message'] = "Mail sent to retailer successfully.";
								}
							}


					}else if($userdata['data']['skipped_registration']){

							$arg['status'] = 0;
							$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
							$arg['error_line'] = __line__;
							$arg['message'] = $this->lang->line('registration_required');
							$arg['data'] = array();
	...
	public function mail_retailer_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true); 
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1 && (!$userdata['data']['skipped_registration'])) {
							...
							$this->form_validation->set_rules('retailer_id', 'Retailer Id', 'required|numeric');
							$this->form_validation->set_rules('company_id', 'Company Id', 'required|numeric');
							// $this->form_validation->set_rules('product_list', 'Product List', 'required');
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$company_id 	= $this->input->post('company_id')?$this->input->post('company_id'):'';
								$retailer_id 	= $this->input->post('retailer_id')?$this->input->post('retailer_id'):'';
								$product_list 	= $this->input->post('product_list')?$this->input->post('product_list'):'';
								...
								if(!empty($product_list) && is_array($product_list)){
									...
									$productNames = '<br/>  <h4>Products Requested & Qunatity :</h4>';

									foreach ($product_list as $key => $value) {
										
										if(!empty($value['product_list_id']) && !empty($value['units'])){

											$condition = array('id' => $value['product_list_id']);
											$forcheck = $this->dynamic_model->getdatafromtable('product_list', $condition);
											$product_id = $forcheck[0]["product_id"];

											$condition = array('id' => $product_id);
											$forcheck2 = $this->dynamic_model->getdatafromtable('manage_products', $condition);
											$product_name = $forcheck2[0]["product_name"];

											$productNames .= "<span> <b>".($key+1).".</b>  ".$product_name." (".$value['units'].") ".$forcheck[0]["product_unit"]." </span><br/>";
								...
								$where1 = array('slug' => 'alert_retailer_by_user');
								$template_data = $this->dynamic_model->getdatafromtable('manage_notification_mail', $where1);
								// $descrpt = '<p>Hi {USERNAME},</p> <p>Thanks for registering on {SITE_TITLE}.</p><p> Please verify your account by click on the Link :</p>'.$productNames;
								$descrpt = $template_data[0]['description'].$productNames;
								$desc_data = str_replace('{USERNAME}', $name, $descrpt);
								// $desc_data = str_replace('{URL}', $url, $desc);
								$desc_send = str_replace('{SITE_TITLE}', SITE_TITLE, $desc_data);
								// $subject = str_replace('{SITE_TITLE}', SITE_TITLE, $desc_data);
								...
								if(!empty($product_list) && is_array($product_list)){
									...
									foreach ($product_list as $key => $value) {
										...
										if(!empty($value['product_list_id']) && !empty($value['units'])){
											...
											$userdata = array('product_list_id' => $value['product_list_id'], 'retailer_mail_id' => $retailer_mail_count, 'units' => $value['units'], 'created_at' => time());

											$retailer_mail_products = $this->dynamic_model->insertdata('retailer_mail_product_list', $userdata);
	...
	public function mail_product_dimension_post()
	{
		$arg = array();
		// $_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
					...
					// print_r($userdata['data']['skipped_registration']);die;
					...
					if ($userdata['status'] == 1 && (!$userdata['data']['skipped_registration'])) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								if (!empty($_FILES['photo']['name'])) {
									...
									$photo = $this->dynamic_model->fileupload('photo', 'uploads/dimension_media','Picture');
									...
									$where1 = array('role_id' => '1');
									$admin = $this->dynamic_model->getdatafromtable('user', $where1);
									$name = $admin[0]["name"];
									$email = $admin[0]["email"];
									...
									$userdata = array('user_id' => $userdata['data']['id'], 'height' => $height, 'width' => $width, 'length' => $length, 'photo' => $photo, 'created_on' => time());

									$dimension_for_ar_model = $this->dynamic_model->insertdata('dimension_for_ar_model', $userdata);


									$where1 = array('slug' => 'alert_admin_product_dimensions_ar');
									$template_data = $this->dynamic_model->getdatafromtable('manage_notification_mail', $where1);
									$descrpt = '<p>Hi {USERNAME},</p> <p>The user {USER_NAME} with email {USER_EMAIL} is requesting for Ar Model By providing following details.<br/>Height : {DIMENSION_HEIGHT}<br/>Width : {DIMENSION_WIDTH}<br/>Length : {DIMENSION_LENGTH}<br/>Photo : {PHOTO_URL}</p>';
									$descrpt = $template_data[0]['description'];
									$desc_data = str_replace('{USERNAME}', $name, $descrpt);
									$desc_data = str_replace('{DIMENSION_HEIGHT}', $height, $desc_data);
									$desc_data = str_replace('{DIMENSION_WIDTH}', $width, $desc_data);
									$desc_data = str_replace('{DIMENSION_LENGTH}', $length, $desc_data);
									$desc_data = str_replace('{PHOTO_URL}', base_url() . 'uploads/dimension_media/'.$photo, $desc_data);
									$desc_data = str_replace('{USER_EMAIL}', $userEmail, $desc_data);
									$desc_data = str_replace('{USER_NAME}', $userName, $desc_data);
									// $desc_data = str_replace('{URL}', $url, $desc);
									$desc_send = str_replace('{SITE_TITLE}', SITE_TITLE, $desc_data);
									// $subject = str_replace('{SITE_TITLE}', SITE_TITLE, $desc_data);
									$subject = str_replace('{SITE_TITLE}', SITE_TITLE, $template_data[0]['subject']);
									...
									if(!$data){
										...
										$arg['error_code'] = REST_Controller::HTTP_OK;
										$arg['error_line'] = __line__;
										$arg['data'] = array('dimension_for_ar_model_id'=>$dimension_for_ar_model);
										$arg['message'] = "Mail sent to admin successfully.";
									}
	...
	public function company_portal_entered_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$arg['message'] = get_form_error($this->form_validation->error_array());
									...
									$where1 = array('user_id' => $userdata['data']['id'], 'company_id'=> $company_id);
									$data = $this->dynamic_model->getdatafromtable('company_enter_count', $where1);
									...
									if(!empty($data) && count($data)>0){
										...
										$userdata = array('user_id' => $userdata['data']['id'], 'company_id' => $company_id, 'created_on' => time());

										$company_enter_count = $this->dynamic_model->insertdata('company_enter_count', $userdata);


										if(!$company_enter_count){
											$arg['status'] = 0;
											$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
											$arg['error_line'] = __line__;
											$arg['message'] = $this->lang->line('server_problem');
	...
	public function companys_showroom_portal_tapped_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$arg['message'] = get_form_error($this->form_validation->error_array());
									...
									$where1 = array('user_id' => $userdata['data']['id'], 'company_id'=> $company_id);
									$data = $this->dynamic_model->getdatafromtable('companys_showroom_portal_tapped', $where1);
									...
									if(!empty($data) && count($data)>0){
										...
										$company_enter_count = $this->dynamic_model->insertdata('companys_showroom_portal_tapped', $userdata);
	...
	public function companys_showroom_entered_count_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							$this->form_validation->set_rules('showroom_id', 'Showroom Id', 'required|numeric');
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$arg['message'] = get_form_error($this->form_validation->error_array());
									...
									$showroom_id 	= $this->input->post('showroom_id')?$this->input->post('showroom_id'):'';


									$where1 = array('user_id' => $userdata['data']['id'], 'showroom_id'=> $showroom_id);
									$data = $this->dynamic_model->getdatafromtable('companys_showroom_entered_count', $where1);
									...
									if(!empty($data) && count($data)>0){
										...
										$userdata = array('user_id' => $userdata['data']['id'], 'showroom_id' => $showroom_id, 'created_on' => time());

										$companys_showroom_entered_count = $this->dynamic_model->insertdata('companys_showroom_entered_count', $userdata);


										if(!$companys_showroom_entered_count){
											$arg['status'] = 0;
											$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
											$arg['error_line'] = __line__;
											$arg['message'] = $this->lang->line('server_problem');
	...
	public function view_showroom_video_count_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$arg['message'] = get_form_error($this->form_validation->error_array());
									...
									$where1 = array('user_id' => $userdata['data']['id'], 'showroom_id'=> $showroom_id);
									$data = $this->dynamic_model->getdatafromtable('view_showroom_video_count', $where1);
									...
									if(!empty($data) && count($data)>0){
										...
										$view_showroom_video_count = $this->dynamic_model->insertdata('view_showroom_video_count', $userdata);


										if(!$view_showroom_video_count){
											$arg['status'] = 0;
											$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
											$arg['error_line'] = __line__;
											$arg['message'] = $this->lang->line('server_problem');
	...
	public function get_requested_model_post() 
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							// $this->form_validation->set_rules('dimension_for_ar_model_id', 'dimension_for_ar_model_id', 'required|numeric');
							$this->form_validation->set_rules('device_type', 'Device Type', 'required');
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$where = array('user_id' => $userID, 'status'=>'Active', 'is_deleted'=>0);
								$findresult = $this->dynamic_model->getdatafromtable('dimension_for_ar_model', $where);
								...
								if(!empty($findresult) && count($findresult)>0){
									...
									foreach ($findresult as $key => $value) {
										...
										$thumbnail = base_url() . 'uploads/dimension_media/'.$value['photo'];

										if($value['is_developed']=='1'){

											if($this->input->post('device_type')=="android"){
												$Url = base_url() . 'uploads/company_media/'.$value['ar_model_android'];
											}else{
												$Url = base_url() . 'uploads/company_media/'.$value['ar_model_ios'];
											}
											$product_name = $value['product_name'];

											array_push($completed, array(
												"dimension_for_ar_model_id" => $value['id'],
												"ar_model_url" => $Url,
												"thumbnail" => $thumbnail,
									...
									$arg['error_code'] = REST_Controller::HTTP_OK;
									$arg['error_line'] = __line__;
									$arg['data'] = array("product_saved"=> $completed, "product_requested"=> $remained);
									$arg['message'] = $this->lang->line('success');
	...
	public function search_companies_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							if (true)  {
								...
								// $data = $this->api_model->get_companies($location,$product,$per_page,$offset);
								$data = $this->api_model->search_companies($search);
	...
	public function get_category_countries_post()
	{
		$arg = array();
		$_POST = json_decode(file_get_contents("php://input"), true);
		$version_result = version_check_helper1();
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							// } else {
								...
								$data = $this->api_model->get_category_countries($category_id);
								...
								if(!$data){
									...
									$arg['data'] = $data;
									// $arg['thumbnail'] = base_url().'uploads/company_media/'.$thumbnail;
									$arg['message'] = "category countries listed successfully.";
								}
							// }
			...
			if ($_POST) {
					...
					if ($userdata['status'] == 1) {
							...
							if ($this->form_validation->run() == FALSE) {
								...
								$filterArr = array();
								$filterArr['country_id'] = $this->input->post('country_id')?$this->input->post('country_id'):'';
								$filterArr['company_id'] = $this->input->post('company_id')?$this->input->post('company_id'):'';
								$filterArr['category_id'] = $this->input->post('category_id')?$this->input->post('category_id'):'';
								$filterArr['product_name'] = $this->input->post('product_name')?$this->input->post('product_name'):'';
								$page = (int)$this->input->post('page');
