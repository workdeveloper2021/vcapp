				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						if ($fullfill_step == 1) {
							...
							$token = strhlp_get_token(array('card_number'	=>	$card_number,
										'expiry_month'		=>	$expiry_month,
										'cvv_no'			=>	$cvv_no,
										'expiry_year'		=>	$expiry_year));
							...
							{
								$res = strhlp_checkout(array('amount'=>$amount,
									'name'	=> $name. ' ' .$lastname,
									'email'	=> $userdata['data']['email'],
									'description' => $amount.' successfully paid!',
									'token'	=>	$token),array('user_id'	=>	$usid),2);
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$newsfeed_comments_id = $this->dynamic_model->insertdata('newsfeed_comments', array('user_id' => $user_id, 'newsfeed_id' => $newsfeed_id,'comments'=>$comments,'added_date' => date('Y-m-d H:i:s')));
			          	...
			          	if($newsfeed_comments_data)
			          	{
			          		for ($i=0; $i <count($newsfeed_comments_data) ; $i++) 
			          		{
				          		...
				          		$comments_data[] = array(
			          			    ...
			          			    'img_url'=>$profile_img,
			          			    'added_date'=>$newsfeed_comments_data[$i]->added_date);
				          	}
