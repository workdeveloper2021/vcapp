...
class Instructor extends REST_Controller
{
	...
	public function nutrition_get()
	{
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$data = $this->dynamic_model->getdatafromtable('manage_nutrition', array('status' => 'Active'), 'id, name, img_url, description');
				...
				if (!$data) {
					...
					array_walk($data, function (&$key) {
						$key["img_url"] = site_url() . 'uploads/nutrition/' . $key['img_url'];
					});
					$arg['status']     = 1;
					$arg['error_code']  = HTTP_OK;
					$arg['error_line'] = __line__;
	...
	public function week_days_get()
	{
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$data = $this->dynamic_model->getdatafromtable('manage_week_days', array(), 'id, week_name');
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$this->form_validation->set_rules(
					...
					'required|trim',
					array(
						'required' => 'Week day id is required'
					)
				);
				$this->form_validation->set_rules(
					'quantity',
					'Quantity',
					'required|trim|numeric|greater_than_equal_to[1]',
					array(
						'required' => 'Quantity is required',
						'greater_than_equal_to' => 'Please enter valid quantity'
					)
				);
				// $this->form_validation->set_rules(
				// 	'age',
				...
				// 		'greater_than_equal_to' => 'Please enter valid age'
				// 	)
				// );
				$this->form_validation->set_rules(
					'calories',
				...
				$this->form_validation->set_rules(
					'description',
					'Description',
					'required|trim|min_length[1]|max_length[140]',
					array(
						'required' => 'Description is required',
						'min_length' => 'Description is required',
						'max_length' => 'Not accepted more then 140 character'
					)
				);
				if ($_FILES['file']['name']) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$checkNutrition = $this->dynamic_model->getdatafromtable('manage_nutrition', array('id' => $nutrition));

					if (!$checkNutrition) {
						$arg['status']  = 0;
						$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
						$arg['error_line'] = __line__;
						$arg['message'] = 'Invalid nutrition';
					} else {
						$week_day_id = $this->input->post('week_day_id');
						$checkWeekDays = $this->db->query('SELECT * FROM `manage_week_days` WHERE `id` IN ('.$week_day_id.')')->num_rows();
						$exp_week_day_id = explode(',', $week_day_id);
						if (count($exp_week_day_id) != $checkWeekDays) {
							$arg['status']  = 0;
							$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
							$arg['error_line'] = __line__;
							$arg['message'] = 'Invalid week days';
						} else {
							$user_id = $userdata['data']['id'];
							...
							$protien = $this->input->post('protien');
							...
							if (!empty($_FILES['file']['name'])) {
								$image = $this->dynamic_model->fileupload('file', 'uploads/meal');
								$insertData['file_url'] = $image;
							}
							$insertId = $this->dynamic_model->insertdata('meals', $insertData);
							$insertWeek = array();
							foreach ($exp_week_day_id as $exp) {
								array_push($insertWeek, array(
									'meal_id'	=>	$insertId,
									'week_id'	=>	$exp
								));
							}
							if (!empty($insertWeek)) {
								$this->db->insert_batch('meal_week_days', $insertWeek);
							}
							$arg['status']     = 1;
							$arg['error_code']  = HTTP_OK;
							$arg['error_line'] = __line__;
	...
	public function get_meal_post()
	{
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					$arg = array();
					$this->form_validation->set_rules('nutrition_id', 'Nutrition Id', 'required');
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$nutrition_id = $this->input->post('nutrition_id');
						if ($this->input->post('id')) {
							$condition = array(
								'meals.created_by' 	=> $user_id,
								'meals.id' 			=> $this->input->post('id'),
								'meals.nutrition_id' => $nutrition_id
							);
						} else {
							$condition = array(
								'meals.created_by' 	=> $user_id,
						...
						$on = 'manage_nutrition.id = meals.nutrition_id';
						...
						$collection = $this->dynamic_model->getTwoTableData('meals.id, meals.nutrition_id, meals.diet_name, meals.age, meals.quantity, meals.calories, meals.carbs, meals.protien, meals.fat, meals.calcium, meals.description, meals.file_url, meals.status, manage_nutrition.name as nutrition_name', 'meals', 'manage_nutrition', $on, $condition, $limit, $offset);
						...
						if (!$collection) {
							...
							array_walk($collection, function (&$key) {
								$key["file_url"] = site_url() . 'uploads/meal/' . $key['file_url'];
							});
							$data = array();
							foreach($collection as $col) {
								$meal_condition = array(
									'meal_week_days.meal_id'	=>	$col['id']
								);
								$mealon = 'manage_week_days.id = meal_week_days.week_id';
								$meal_week = $this->dynamic_model->getTwoTableData('manage_week_days.id, manage_week_days.week_name', 'meal_week_days', 'manage_week_days', $mealon, $meal_condition);
								$col['week_data'] = $meal_week;
								array_push($data, $col);
							}
	...
	public function update_meal_post($id)
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if (!$checkNutrition) {
						...
						if (count($exp_week_day_id) != $checkWeekDays) {
							...
							$checkRecord = $this->dynamic_model->getdatafromtable('meals', array('id' => $id, 'created_by' => $user_id), '*');

							if (!$checkRecord) {
								$arg['status']  = 0;
								$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
								$arg['error_line'] = __line__;
								$arg['message'] = 'Invalid meal';
							} else {
								$diet_name = $this->input->post('diet_name');
								$age = 0; // $this->input->post('age');
								$quantity = $this->input->post('quantity');
								$calories = $this->input->post('calories');
								$carbs = $this->input->post('carbs');
								$protien = $this->input->post('protien');
								...
								if (!empty($_FILES['file']['name'])) {
									$image = $this->dynamic_model->fileupload('file', 'uploads/meal');
									$updateData['file_url'] = $image;
								}
								$this->dynamic_model->updateRowWhere('meals', array('id' => $id), $updateData);
								$this->dynamic_model->deletedata('meal_week_days', array('meal_id' => $id));
								$insertWeek = array();
								foreach ($exp_week_day_id as $exp) {
									array_push($insertWeek, array(
										'meal_id'	=>	$id,
	...
	/* ************************************************
	Get Category by business
	************************************************ */
	public function business_category_get($business_id)
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
