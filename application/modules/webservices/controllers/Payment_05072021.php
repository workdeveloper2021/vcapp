...
class Payment extends REST_Controller {
  ...
  public function clover_card_save_post() {
	...
	public function cardGet_bk_post()
  {
     $arg   = array();
     $_POST = json_decode(file_get_contents("php://input"), true);
     ...
     /*$where = array('user_id' => $userid);
       ...
       if(!empty($result)){
        ...
        if (!empty($result)) {
          foreach ($result as $key => $value) {
            ...
            $data[] = array('customer_code' => $value['profile_id'],
                            ...
                            'number' => decode($value['card_no']),
                            'expiry_month' => decode($value['expiry_month']),
                            'expiry_year' => decode($value['expiry_year']),
      ...
      if($card_data)
      {
              $where = array('id' => $userid, 'status' => 'Active');
              ...
              $clover_customer_profile_id =  $user_data[0]['clover_customer_profile_id']; //'D5TATZMAD6QQW';
 ...
 public function cardDelete_bk_post()
  {
     ...
     $userid = decode($this->input->post('userid')); 
      ...
      if($this->form_validation->run() == FALSE)
      {
          ...
          $this->dynamic_model->deletedata('user_card_save',$checkfav); 

          }else{
               $status = 0;
              $message = $res['message'];
          }*/
          $where = array('clover_customer_profile_id' => $clover_customer_profile_id, 'status' => 'Active');
          $user_data = $this->dynamic_model->getdatafromtable('user', $where);
          $marchant_id  = $user_data[0]['marchant_id'];
          $country_code = $user_data[0]['marchant_id_type'];
          $clover_key   = $user_data[0]['clover_key'];
          ...
          $this->dynamic_model->updateRowWhere('user_card_save', array('card_id'=>$card_id,'profile_id'=>$clover_customer_profile_id,'user_id'=>$userid), array('id_deleted'=>1));
