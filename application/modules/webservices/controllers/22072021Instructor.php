			...
			if ($userdata['status'] != 1) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if (!$checkNutrition) {
						...
						$checkWeekDays = $this->dynamic_model->getdatafromtable('manage_week_days', array('id' => $week_day_id));
						if (!$checkWeekDays) {
							$arg['status']  = 0;
							$arg['error_code'] = REST_Controller::HTTP_NOT_FOUND;
							$arg['error_line'] = __line__;
							...
							$this->dynamic_model->insertdata('meals', $insertData);
							$arg['status']     = 1;
							$arg['error_code']  = HTTP_OK;
							$arg['error_line'] = __line__;
							$arg['message']    = $this->lang->line('record_found');
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$on2 = 'manage_week_days.id = meals.week_day_id';
						
						$collection = $this->dynamic_model->getThreeTableData('meals.id, meals.nutrition_id, meals.diet_name, meals.week_day_id, meals.age, meals.quantity, meals.calories, meals.carbs, meals.protien, meals.fat, meals.calcium, meals.description, meals.file_url, meals.status, manage_nutrition.name as nutrition_name, manage_week_days.week_name', 'meals', 'manage_nutrition', 'manage_week_days', $on, $on2, $condition);
