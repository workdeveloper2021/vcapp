...
class Email_template extends My_Controller {
	...
	public function __construct(){        
      ...
      public function index(){
        check_permission(VIEW,"manage_email_template",1);
       $header['title']  = $this->lang->line('template_email');
       $data['email_data'] = $this->dynamic_model->getdatafromtable(TABLE_EMAIL_TEMPLATE);
       $this->admintemplates('template/email_template_list',$data,$header);
         
    }
    public function editMail($mail_id=''){
        check_permission(EDIT,"manage_email_template",1);
        $header['title']  = $this->lang->line('template_email_edit');
        $wherepage = array('id' => decode($mail_id));
        $data['email_data'] = $this->dynamic_model->getdatafromtable(TABLE_EMAIL_TEMPLATE,$wherepage);
        $this->admintemplates('template/email_template_edit',$data,$header);
    }
    public function editMailAction(){
        ...
        if (isset($is_submit) && $is_submit == 1) {
            ...
            if($this->form_validation->run() == FALSE) {  
                  ...
                  redirect(base_url('admin/email_template/editMail/'.$template_id));
            }else{
                $upWhere = array('id' =>decode($template_id));
                $time=time();
                ...
                $updatedata['send_by']     = $this->login_user_id;
                $updatedata['create_dt']   =  $time;
                $updatedata['update_dt']   =  $time;
                
                $this->dynamic_model->updateRowWhere(TABLE_EMAIL_TEMPLATE,$upWhere,$updatedata);       
                $this->session->set_flashdata('updateclass', 'success');
                $this->session->set_flashdata('updateerror', $this->lang->line('template_updatemsg'));
                redirect(base_url('admin/email_template/editMail/'.$template_id));
                 }else{
                        $this->session->set_flashdata('updateclass', 'danger');
                        $this->session->set_flashdata('updateerror', 'Description field is required');
                       redirect(base_url('admin/email_template/editMail/'.$template_id));
                }        
            } 
 ...
 //                if($data){
 //                    // if(verify_password_hash($this->input->post('userpass'), $data['user_pass']) == 'verified'){
 //                    if(encrypt_password($this->input->post('userpass')) == $data['user_pass']){
 ...
 //                        $session_array['session_userrole'] = $data['user_role'];
 ...
 //                );
 //                $data = array(
 //                    'user_pass' => encrypt_password($newpassword)
 //                );
 //                $update = $this->dynamic_model->updateRowWhere(TABLE_USERS, $where1, $data);

 //                $data['subject']     = 'FORGOT YOUR PASSWORD ?'; 
 //                $data['description'] = "Hi $name, <br /> To make your account secure, change your password after login with the above details. If you didn’t make the request, just ignore the email. <br /> No worry, we got you! Log into your account using the details below.";
 ...
 //                sendEmailCI("$email", SITE_NAME ,'Forgot Password From PosiTooT', $msg); 
 ...
 //            if(encrypt_password($this->input->post('userpass')) == $loguserinfo['user_pass']){
 //                $detailsData = $this->session->userdata('logged_in');
 ...
 //                } else {
 //                    $this->session->set_flashdata('updateclass', 'danger');
 //                    $this->session->set_flashdata('updateerror', $this->lang->line('password_not'));
 //                    redirect(site_url().'admin/profile');                     
 //                }
 //            }           
 //        } else {
 ...
 //        if(isset($is_submit) && $is_submit == 1){
 //            $this->form_validation->set_rules('updateoldpass', 'updateoldpass', 'required' , array( 'required' => $this->lang->line('password_required')));
 //            $this->form_validation->set_rules('updatenewnm', 'updateoldpass', 'required', array( 'required' => $this->lang->line('new_password_required')));
 ...
 //                $userdata = $this->dynamic_model->get_user($userid);
 //                if(encrypt_password($oldpass) == $userdata['user_pass']){
 //                    $updatedata['user_pass'] = encrypt_password($newpass);
 //                    $this->dynamic_model->updatedata(TABLE_USERS, $updatedata, $userid);
