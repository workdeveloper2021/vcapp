...
class Request_ar_model extends My_Controller {
    ...
    public function __construct(){      
        ...
        $this->load->model('request_ar_model_model');
    ...
    public function index() {
        ...
        $header['title'] = $this->lang->line('request_ar_list'); 
        $data['userdata']=$this->dynamic_model->getdatafromtable(TABLE_USERS,array("role_id "=>3),'id');       
        $this->admintemplates('request_ar_model/request_list',$data, $header);
    }

    public function requestArAjaxlist(){
        ...
        $totalRecord      = $this->request_ar_model_model->requestArAjaxlist(true);
        $getRecordListing = $this->request_ar_model_model->requestArAjaxlist(false,$start,$length, $column_name, $order[0]['dir']);
        ...
        if(!empty($getRecordListing)) {
            ...
            foreach($getRecordListing as $recordData) {
                    ...
                    $profile_url = base_url('admin/request_ar_model/createModel/').$login_user_id;                    
                    
                    $recordListing[$i][0]=   '<input type="checkbox" name="checkAll[]" value="'.($recordData->id).'" class="cb-element" ">';
                    $recordListing[$i][1]=  $srNumber+1;
                    if(!empty($recordData->photo)){
                    $user_pic = base_url('uploads/dimension_media/').'/'.$recordData->photo;
                     }else{
                         $user_pic = '';
                     }
                    $recordListing[$i][2]= $recordData->name;
                    $recordListing[$i][3]= $recordData->email;
                    $recordListing[$i][4]= $recordData->height;
                    $recordListing[$i][5]= $recordData->width;
                    $recordListing[$i][6]= $recordData->length;
                    $recordListing[$i][7]= $user_pic!=''?'<img src="'.$user_pic.'" width="40" height="40">':'';
                    $recordListing[$i][8]= get_formated_date($recordData->created_on, 2);
                   
                    $table = 'dimension_for_ar_model';
                    $field = 'status';
                    $urls  =  base_url('admin/request_ar_model/updateStatus'); 
                    $actionContent='';
                    ...
                    $actionContent = '';
                    if($recordData->is_developed=='0'){
                        $actionContent .='<a href="'.$profile_url.'" title="Upload Models" class="btn btn-success">Upload Models</a> '; 
                     }else{
                        $actionContent .= 'UPLOADED SUCCESSFULLY';
                     }
    ...
    /* Show Profile info */
    public function createModel($user_id=''){
        // check_permission(EDIT,"user_list",1);
        $uid =  decode($user_id);
        if(!empty($user_id) && !empty($uid)){
            $loguserinfo['reqId'] = $uid;
            $header['title'] = $this->lang->line('tb_upload_req_model');
            $this->admintemplates('request_ar_model/request_model_upload', $loguserinfo, $header);
        } else{
            redirect(base_url('admin/request_ar_model'));
        }
    }
    ...
    public function uploadReqModels(){
        // check_permission(EDIT,"user_list",1);
        extract($this->input->post());
        // $allowedExts = array("JPG","JPEG","PNG","png","jpeg","jpg");
        $is_submit = $this->input->post('is_submit');
        if(isset($is_submit)){
            ...
            if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('updateclass', 'danger');
                $this->session->set_flashdata('updateerror', get_form_error($this->form_validation->error_array()));
                redirect(site_url().'admin/request_ar_model');
            } else {

                    $updatedata = array();


                    $file_ext = pathinfo($_FILES['usdzFile']["name"], PATHINFO_EXTENSION);
                    if (!empty($_FILES['usdzFile']['name'])) {

                        $file_ext=strtolower($file_ext);
                        $usdz3dmodel = $this->dynamic_model->fileupload('usdzFile', 'uploads/company_media', 'Model');
                    ...
                    $file_ext = pathinfo($_FILES['glbFile']["name"], PATHINFO_EXTENSION);
                    if (!empty($_FILES['glbFile']['name'])) {

                        $file_ext=strtolower($file_ext);
                        $glb3dmodel = $this->dynamic_model->fileupload('glbFile', 'uploads/company_media', 'Model');
                    ...
                    if($glb3dmodel=='' || $usdz3dmodel==''){

                         $this->session->set_flashdata('updateclass', 'danger');
                         $this->session->set_flashdata('updateerror', $this->lang->line('tb_3d_model_required'));
                        redirect(site_url().'admin/request_ar_model');                    
                        ...
                        $updatedata['ar_model_android'] = $glb3dmodel;
                        $updatedata['product_name'] = $this->input->post('product_name');
                        $updatedata['ar_model_ios'] = $usdz3dmodel;
                        $updatedata['is_developed'] = '1';

                        $condition= array('id'=>$reqId);
                        $this->dynamic_model->updateRowWhere('dimension_for_ar_model',$condition,$updatedata);


                        $this->session->set_flashdata('updateclass', 'success');
                        $this->session->set_flashdata('updateerror', $this->lang->line('tb_upload_req_success'));
                        redirect(site_url().'admin/request_ar_model');  
