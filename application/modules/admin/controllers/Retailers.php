...
class Retailers extends My_Controller {
    ...
    public function __construct(){      
        ...
        $this->load->model('retailers_model');
    ...
    public function companyRetailers($user_id=''){
        // check_permission(EDIT,"user_list",1);
        $uid =  decode($user_id);
        if(!empty($user_id) && !empty($uid)){
            $loguserinfo['cid'] = $uid;
            $header['title'] = $this->lang->line('title_retailers_list');
            $this->admintemplates('retailers/retailers_list', $loguserinfo, $header);
        } else{
    ...
    public function retailersAjaxlist($cid){
        ...
        $totalRecord      = $this->retailers_model->retailersAjaxlist(true,0,0,'','desc',$cid);
        $getRecordListing = $this->retailers_model->retailersAjaxlist(false,$start,$length, $column_name, $order[0]['dir'],$cid);
        ...
        if(!empty($getRecordListing)) {
            ...
            foreach($getRecordListing as $recordData) {
                    ...
                    $recordListing[$i][4]= $recordData->country_name;
                    $recordListing[$i][5]= $recordData->city;


                    $where = "retailer_id ='".$recordData->id."' and company_id ='".$cid."' and is_deleted='0'";
                    $product_count=$this->dynamic_model->getdatafromtable('retailer_mail_count',$where); 

                    $recordListing[$i][6]= !empty($product_count)?count($product_count):0; 


                    // $recordListing[$i][6]= $recordData->city;

                   
                    $table = 'company_retailers';
                    $field = 'status';
                    $urls  =  base_url('admin/retailers/updateStatus'); 
                    $actionContent='';
                    ...
                    $profile_url = base_url('admin/retailers/retailerprofile/').$login_user_id;                    
                    $actionContent = '';
    ...
    /* Show Profile info */
    public function retailerprofile($user_id=''){
        // check_permission(EDIT,"user_list",1);
        $uid =  decode($user_id);
        if(!empty($user_id) && !empty($uid)){
            ...
            $loguserinfo['locations'] = $locations;
                ...
                $where = "id ='".$uid."'";
                $userdata=$this->dynamic_model->getdatafromtable('company_retailers',$where); 
                // $where = "status ='Active'";
                // $locations=$this->dynamic_model->getdatafromtable('manage_comapny_location',$where); 

            $loguserinfo['retailerinfo'] = $userdata;
            $header['title'] = $this->lang->line('btn_update_details');
            $this->admintemplates('retailers/retailer-update', $loguserinfo, $header);
        } else{
            redirect(base_url('admin/companies'));
        }
    }

    public function addRetailers($cid){
        ...
        // manage_showroom_list
            ...
            $loguserinfo['cid'] = $cid;
            $header['title'] = $this->lang->line('title_add_company_retailer');
            $this->admintemplates('retailers/retailer-add', $loguserinfo, $header);
    }
    ...
    public function retailerUpdate(){
        ...
        if(isset($is_submit) && $is_submit == 1){
            ...
            $this->form_validation->set_rules('retail_name', 'Name', 'required');
            $this->form_validation->set_rules('retail_email', 'Email', 'required');
            $this->form_validation->set_rules('retailer_country', 'Country', 'required');
            $this->form_validation->set_rules('retail_city', 'City', 'required');
            ...
            if ($this->form_validation->run() == FALSE){
                ...
                redirect(site_url().'admin/retailers/retailerprofile/'.encode($updateid));
            } else {
                $updatedata = array();                
                $userid = $updateid;


                $updatedata['name'] = trim($retail_name);
                $updatedata['country'] = trim($retailer_country);
                $updatedata['city'] = trim($retail_city);
                $updatedata['email'] = trim($retail_email);
                $this->dynamic_model->updatedata('company_retailers', $updatedata, $userid); 
                $this->session->set_flashdata('updateclass', 'success');
                $this->session->set_flashdata('updateerror', $this->lang->line('retailer_update'));
                redirect(site_url().'admin/retailers/retailerprofile/'.encode($updateid));  
            }           
        } else {
             $this->session->set_flashdata('updateclass', 'danger');
    ...
    /* User Profile update by admin */
    public function retailerAddSubmit($cid){
        ...
        if(isset($is_submit) && $is_submit == 1){
            ...
            if ($this->form_validation->run() == FALSE){
                $this->session->set_flashdata('updateclass', 'danger');
                $this->session->set_flashdata('updateerror', get_form_error($this->form_validation->error_array()));
                redirect(site_url().'admin/retailers/addRetailers/'.$cid);
            } else {
                $updatedata = array();
                ...
                $this->session->set_flashdata('updateclass', 'success');
                $this->session->set_flashdata('updateerror', $this->lang->line('retailer_add'));
                redirect(site_url().'admin/retailers/addRetailers/'.$cid);  
            }           
        } else {
             $this->session->set_flashdata('updateclass', 'danger');
