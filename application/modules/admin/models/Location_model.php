...
class Location_model extends CI_model{
    ...
    public function locationAjaxlist($isCount=false,$start=0,$stop=0, $column_name='',$order='desc'){
        if(!empty($column_name) && $column_name=='location' ){
        ...
        //--------search text-box value start
         if(!empty($search['value'])){
            ...
            $this->db->where('(`location` LIKE "%'.$search_info.'%" )',NUll);
        }
