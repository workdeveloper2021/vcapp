                ...
                <div class="row">
                    ...
                    <?php  for ($i=0; $i <count($product) ; $i++) {  ?> 
                      <div class="col-md-12">
                        ...
                        <div style="padding:15px 0px" class="row showcod<?= $i+1; ?> removecla<?= $i+1; ?>">
                            <input type="hidden" name="nos360[]" value="">
                            <div class="col-sm-4">
                               X : <input type="number" required step="0.00000001" name="xval<?= $i+1; ?>" style="width:20%" value="<?= $product[$i]['xval'] ?>"> 
                               Y : <input type="number" required step="0.00000001" name="yval<?= $i+1; ?>" style="width:20%" value="<?= $product[$i]['yval'] ?>"> 
                               Z : <input type="number" required step="0.00000001" name="zval<?= $i+1; ?>" style="width:20%"  value="<?= $product[$i]['zval'] ?>">
                            </div>
                            <div class="col-sm-3">
                              <textarea required style="width: 1200px;" name="coordinate_360_info<?= $i+1; ?>" class="form-control" placeholder="Info text"><?= $product[$i]['info']?></textarea>
                            </div>
                            <div class="col-sm-2">
                              <input type="text" class="form-control" name="product_name<?= $i+1; ?>" placeholder="Product Name" value="<?= $product[$i]['product_name']?>">
                            </div>
                            <div class="col-md-3">
                              <label for="inputName1" class="control-label">Images</label>
                              <input type="file" style="width:247px" name="image<?= $i+1; ?>[]" class="form-control" id="inputName1" multiple accept="image/x-png,image/gif,image/jpeg" />
                            </div>
                            <div id="education_fields<?= $i+1; ?>">
                              <div class="row">
                                <div class="col-md-4">
                                  <label for="inputName1" class="control-label">3D Modals</label>
                                  <input type="file" style="width:247px" name="3dmodals<?= $i+1; ?>" class="form-control" id="inputName1" />
                                </div>
                                <div class="col-md-4">
                                  <label for="inputName1" class="control-label">Modals Color</label>
                                  <input type="color" style="width:247px" name="modals_color<?= $i+1; ?>" class="form-control" id="inputName1" />
                                </div>
                                <div class="col-md-4">
                                  <button style="margin: 30px 21px; float: left;" class="btn btn-success" type="button" onclick="education_fields(<?= $i+1; ?>);">+</button>
                                </div>
                              </div>
                            </div>

                            <div class="col-sm-12" style="text-align:right">
                              <button class="btn btn-danger" type="button" onclick="remove_education_fields2(<?= $i+1; ?>);"> Remove</button>
                            </div>
