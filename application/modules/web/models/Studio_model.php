...
class Studio_model extends CI_Model {
	...
	public function get_calendar_scheduled_class_list($business_id='',$from_date='',$to_date='',$class = array(), $room = array(), $instructor = array()) {
		...
		$this->db->where('class_scheduling_time.business_id',$business_id);
		$this->db->where('class_scheduling_time.scheduled_date >=', $from_date);
		$this->db->where('class_scheduling_time.scheduled_date <=', $to_date);

		if (!empty($class)) {
			$this->db->where('class_scheduling_time.class_id', decode($class[0]));
		}

		if (!empty($room)) {
			$this->db->where('class_scheduling_time.location_id', decode($room[0]));	
		}

		if (!empty($instructor)) {
			$this->db->where('class_scheduling_time.instructor_id', $instructor[0]);	
		}
	...
	public function get_scheduled_class_detail($business_id='',$class_id='',$scheduled_date='', $scheduled_id=''){
		...
		$url=base_url().'uploads/user/';
		$this->db->select("business_class.*,class_scheduling_time.from_time,class_scheduling_time.to_time,class_scheduling_time.location_id,class_scheduling_time.day_id,class_scheduling_time.instructor_id,class_scheduling_time.scheduled_date,class_scheduling_time.class_id,business_location.location_name, business_location.location_url as web_link, business_location.map_url as location_url, business_location.capacity,user.name,user.lastname,CONCAT('". $url ."', user.profile_img) as profile_image,manage_week_days.week_name, class_scheduling_time.status as scheduling_status");
		$this->db->from('class_scheduling_time');
		$this->db->join('business_class','business_class.id=class_scheduling_time.class_id','LEFT');
