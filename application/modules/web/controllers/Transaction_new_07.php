...
class Transaction_new extends MX_Controller {
                              ...
                              if($curr_date > $sub_end){
									...
									$msg = $this->lang->line('subscription_succ');
										...
										else
										{
		                                  ...
		                                  $startdate  = date('Y-m-d 00:00:00');
		                                  $start_date = strtotime($startdate."+".($days+1)." days");
		                                  $sdate=date('d M Y',$start_date);
		                                  if($subplan_type==1){
							            		 ...
							            		 $end_date   = strtotime(date('Y-m-d 23:59:59',strtotime($startdate."+30 days")));
							            		 $edate      = date('d M Y',$end_date);
							            	}elseif($subplan_type==2){
							                     ...
							                     $end_date   = strtotime(date('Y-m-d 23:59:59',strtotime($startdate."+180 days")));
							                    $edate       = date('d M Y',$end_date);
							                }else{
					                            ...
					                            $end_date   = strtotime(date('Y-m-d 23:59:59',strtotime($startdate."+365 days")));
					                            $edate      = date('d M Y',$end_date);
							                }
										}
	...
	public function subscription_plan_recurring()
	{
        ...
        $previous_date = strtotime(date('Y-m-d 23:59:59'));

        //$transaction_distinct_user =$this->Mod_Common->customQuery("select distinct(user_id) as user_id from tbl_transaction_point_history where rewards_type=1 and is_check_point_history=0 and FROM_UNIXTIME(expiry_date,'%Y-%m-%d') < '".$previous_date."' order by transaction_point_id ASC");

        $transaction_distinct_user =$this->Mod_Common->customQuery("select distinct(user_id) as user_id from tbl_transaction_point_history where rewards_type=1 and is_check_point_history=0 and expiry_date <= '".$previous_date."' order by transaction_point_id ASC");
        if(count($transaction_distinct_user)>0)
        {
            foreach ($transaction_distinct_user as $key => $val_user_data) 
            {
