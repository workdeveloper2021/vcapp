 ...
 * ********************************************************** */
class Studiotest extends MX_Controller {
			...
			{ 
			  ...
			  if($service_type==1){
		        ...
		        $class_type=!empty($business_data) ? $business_data[0]['class_type']:'';
                 ...
                 $where=array("business_id"=>decode($userdata['data']['business_id']));
				}elseif($service_type==2){
					...
					if($this->form_validation->run() == FALSE)
					{
						...
						if ($couter) {
							...
							foreach($client_details as $value) {
								...
								$purchase_passes_data = $this->dynamic_model->getQueryResultArray('SELECT business_passes.id, business_passes.pass_name, business_passes.pass_validity, business_passes.pass_type, business_passes.pass_type_subcat, user_booking.passes_start_date as start_date, user_booking.passes_end_date as end_date, user_booking.sub_total FROM `user_booking` JOIN business_passes on (business_passes.id = user_booking.service_id) WHERE user_booking.business_id = '.$business_id.' and user_booking.service_type = 1 and user_booking.user_id = '.$user_id.' and passes_status = 1 AND user_booking.status != "Pending"');
								...
								if(!empty($class_id)){
									...
									$queryPasses = "SELECT (SELECT id FROM user_booking as u where u.service_id = buss.id AND u.status = 'Pending' AND u.user_id = '".$user_id."' and u.service_type = '1') as added_incart, buss.id, buss.pass_name, buss.pass_type, buss.pass_type_subcat, buss.purchase_date, buss.pass_validity, buss.amount, buss.pass_id, buss.tax1, buss.tax2, buss.tax1_rate, buss.tax2_rate from business_passes_associates AS pass JOIN business_passes as buss on (buss.id = pass.pass_id) WHERE pass.business_id = ".$business_id." AND buss.status = 'Active' AND buss.is_client_visible = 'Yes' GROUP by pass.pass_id";
								}
        ...
        if(!empty($business_data)){
            foreach($business_data as $value) 
            {
                ...
                foreach($cart_data as $value1) 
               {
					...
					$cartData['tax']  = floatVal($value1['tax_amount']);
			...
			return  $result	=	array(
				...
				"total_amount"		=> floatVal(round($total_amt,2)),
				"total_discount"	=> floatVal(round($total_discount,2)),
				"tax"				=> floatVal(round($tax,2)),
				"grand_total"		=> floatVal(round($grand_total,2))
			);
			...
			if ($_POST) {
				...
				if($this->form_validation->run() == FALSE)
				{
					...
					if (count($data) > 0) {
						...
						$res = $this->bomborapay->payment_checkout('POST',$payUrl,$payment_data);
								...
								foreach($cart_data as $value) {
									...
									if($value['service_type']=='1') { 
										...
										$booking_data =   array(
											...
											'passes_end_date'       => $pass_end_date,
											'passes_status'         => $pass_status,
											'passes_total_count'    =>  $validity * 5,
											'passes_remaining_count'  =>  $validity * 5,
											'update_dt'             => $time
										);
										$booking_id = $this->dynamic_model->updateRowWhere('user_booking',$where2,$booking_data);
									} else {
				...
				if($_POST)
		  		{
					...
					if($this->form_validation->run() == FALSE)
					{
						...
						$where= ' user_id = "'.$user_id.'" AND business_id = "'.$business_id.'" AND service_id = "'.$pass_id.'" AND status = "Success" AND passes_status = "1" ';
				...
				if($_POST)
		  		{
					...
					if($this->form_validation->run() == FALSE)
					{
						...
						$business_id 	=	decode($userdata['data']['business_id']);
						$class_id 		=	decode($this->input->post('class_id'));
						$where = array('user_id' => $usid, 'business_id' => $business_id, 'id' => $class_id);
						$result = $this->dynamic_model->getdatafromtable('business_class', $where);
						if (false) {
							$arg['status']  = 0;
							$arg['error_code'] = 0;
							$arg['error_line']= __line__;
							...
							$this->dynamic_model->updateRowWhere('business_class', $whereArray, $status);
							$arg['status']    = 1;
							$arg['error_code'] = REST_Controller::HTTP_OK;
							$arg['error_line']= __line__;
