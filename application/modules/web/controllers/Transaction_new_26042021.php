	...
	public function subcription_plan_purchase()
	{
		...
		if ($version_result['status'] != 1) {
			...
			if ($_POST) {
				...
				$stripe_res = $this->stripe->checkout($data, array('userid'=> $this->input->post('user_id')), 2);
				if ($stripe_res['status'] == 0) {
					
					$arg['status'] = 0;
					$arg['message'] = 'Invalid token';
					$arg['resp'] = $this->input->post();
					$arg['data'] = $this->stripe->getcard('card_1IkmdtA1R4jClJNnDs5nvb3z');
