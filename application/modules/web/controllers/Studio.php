...
class Studio extends MX_Controller
{
				...
				if ($role_check) {
					...
					if ($newuserid) {
						...
						if ($this->input->post('plan')) {
							$planId = decode($this->input->post('plan'));
							$planInfo = $this->db->get_where('subscribe_plan', array('id' => $planId))->row_array();
							if (!empty($planInfo) && $planInfo['amount'] == '0.00') {
								$subplan_type = $planInfo['type'];
								$transaction_data = array(
									'user_id'                => $newuserid,
									'amount'                 => $planInfo['amount'],
									'new_amount'				=> $planInfo['amount'],
									'trx_id'       		    => $time,
									'transaction_type'      => 1,
								...
								$subscription_data = array(
									...
									'sub_end'       		=> $end_date,
									'max_users_count'		=> $planInfo['max_users'],
									'create_dt'        		=> $time,
									'plan_status'        	=> $plan_status,
									'transaction_id'        => $trx_id,
									'update_dt'        		=> $time
								);
								$this->dynamic_model->insertdata('subscription', $subscription_data);
								$where1 = array('id' => $newuserid);
	...
	public function register_business_profile()
	{
		...
		if ($version_result['status'] != 1) {
			...
			if ($this->form_validation->run() == FALSE) {
				...
				//Check Subscription plan purchase or not
				$where = array('id' => $user_id);
				$user_data = $this->dynamic_model->getdatafromtable('user', $where, 'id,plan_id,email_verified');
				if (!empty($user_data)) {
					$plan_id = (!empty($user_data[0]['plan_id'])) ? $user_data[0]['plan_id'] : "";
					$plan_data = $this->studio_model->plan_check($plan_id, $user_id);
					if (empty($plan_data)) {
					...
					if ($user_data[0]['email_verified'] == 0) {
						$arg['status']     = 0;
						$arg['error_code']  = HTTP_NOT_FOUND;
						$arg['error_line'] = __line__;
						$arg['data']       = array();
						$arg['message']    = 'Please go to your inbox and look for an email from Warrior SMS. Click on the Verify Your Email.';
						echo json_encode($arg);
						exit();
					}
				}
	...
	public function get_business_profile()
	{
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$condition = "business.id='" . $business_id . "' AND business.user_id = " . $user_id;
				$business_data = $this->dynamic_model->getTwoTableData('business.*, concat(user.name, " ", user.lastname) as owner_details', 'business', 'user', $on, $condition);
	...
	public function complete_merchant_profile()
	{
		...
		if ($version_result['status'] != 1) {
			...
			$this->form_validation->set_rules('month_start', 'Month Start', 'required|numeric|less_than_equal_to[12]|greater_than[0]|min_length[2]', array(
				'required' => $this->lang->line('start_month_required'),
	...
	public function get_workshop_passes()
	{
		...
		if ($userdata['status'] != 1) {
			...
			$usid = decode($userdata['data']['id']);
			$business_id = decode($userdata['data']['business_id']);
			$pass_data = $this->dynamic_model->getdatafromtable(
				'business_passes',
				array(
					'business_id'   =>   $business_id,
					'service_type'  =>   2,
		...
		if ($userdata['status'] != 1) {
			...
			if ($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$business_id = decode($userdata['data']['business_id']);
					$time		=	time();
					$this->dynamic_model->insertdata(
						'business_shift_instructor_comments',
						array(
			...
			if ($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					for ($i = 0; $i < count($slots); $i++) {
						...
						if (@$rowInfo['updateId']) {
							$this->dynamic_model->updateRowWhere('business_workshop_schdule', array('id' => $rowInfo['updateId']), $updateSchedule);
							$workShopScheduleId = $rowInfo['updateId'];
						} else {
							$updateSchedule['workshop_id'] = $workShopId;
							$updateSchedule['create_dt'] = $time;
							$workShopScheduleId = $this->dynamic_model->insertdata('business_workshop_schdule', $updateSchedule);
							...
							if (!empty($cart_data)) {
								...
								foreach ($cart_data as $value) {
									...
									if ($value['service_type'] == '1') {
										...
										$product_id = $this->dynamic_model->updateRowWhere(
											'business_product',
											array(
												'id'	=>	$value['service_id']
											),
			...
			if ($_POST) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					if (count($data) > 0) {
						...
						$userdata = $this->dynamic_model->getdatafromtable('user', array('id' => $usid));
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$checkStatus = $this->dynamic_model->updateRowWhere(
							'service_instructor',
							array(
								'service_id'    =>  $service_id,
								'instructor_id' =>  $instructor_id
	...
	/* Get Studio Master record for plan */
	public function studio_plan_master()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			$condition = array('status' => 'Active');
			$studio_plan = $this->dynamic_model->getdatafromtable("studio_plan_master", $condition, 'id, name, price');
			if (!empty($studio_plan)) {
				$arg['status']     = 1;
				$arg['error_code']  = HTTP_OK;
				$arg['error_line'] = __line__;
				$arg['data']       = $studio_plan;
				$arg['message']    = $this->lang->line('record_found');
			} else {
				$arg['status']     = 0;
				$arg['error_code']  = HTTP_NOT_FOUND;
	...
	public function get_studio_plan_list()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$condition = array('studio_plan_details.business_id' => $business_id);
				$on = 'studio_plan_master.id = studio_plan_details.plan_id';
				$client_data = $this->dynamic_model->getTwoTableData('studio_plan_details.id, studio_plan_details.plan_id, studio_plan_master.name, studio_plan_details.price, studio_plan_details.status', 'studio_plan_details', 'studio_plan_master', $on, $condition);

				if ($client_data) {

					array_walk($client_data, function (&$key) {
						$key['status'] = ($key['status'] === 'Active') ? "1" : "0";
					});
					$arg['status']    = 1;
					$arg['error_code'] = HTTP_OK;
					$arg['error_line'] = __line__;
	...
	public function save_studio_plan()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$request = $this->input->post('plan');

						$plan_details = $this->dynamic_model->getdatafromtable('studio_plan_master', array('status' => 'Active'), 'GROUP_CONCAT(id) AS id');

						$studio_plan_details = $this->dynamic_model->getdatafromtable('studio_plan_details', array('business_id' => $business_id));


						$insertData = array();
						$updateData = array();
						$planId = explode(',', $plan_details[0]['id']);

						foreach ($request as $req) {
							if (in_array($req['plan_id'], $planId)) {
								if (!$studio_plan_details) {
									array_push($insertData, array(
										'business_id'	=>	$business_id,
										'plan_id'		=>	$req['plan_id'],
										'price'			=>	$req['price'],
									...
									$plan_id = $req['plan_id'];
									$status = false;
									foreach ($studio_plan_details as $details) {
										if ($details['plan_id'] == $plan_id) {
											$status = true;
						...
						if (!empty($insertData) || !empty($updateData)) {
							if (!empty($insertData)) {
								$this->db->insert_batch('studio_plan_details', $insertData);
							}

							if (!empty($updateData)) {
								$this->db->update_batch('studio_plan_details', $updateData, 'id');
							}
							$arg['status']     	= 1;
							$arg['error_code']  = REST_Controller::HTTP_OK;
							$arg['error_line'] 	= __line__;
				...
				// $where = 'business_id = '.$business_id;
				// $client_data = $this->dynamic_model->getdatafromtable('studio_plan_details', $where, 'id, name, price, status');
				// if ($client_data) {
	...
	public function studio_category()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				$collection = get_studio_business_category($business_id);
	...
	public function get_video_master()
	{
		$arg = array();
		$version_result = version_check_helper1();
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$collection = cl_get_video_master($pageid, $category, $subcategory);
		...
		if ($version_result['status'] != 1) {
			...
			if ($userdata['status'] != 1) {
				...
				if ($_POST) {
					...
					$this->form_validation->set_rules('category_id', 'Category', 'required|numeric', array(
						'required' => 'Category is required',
					));
					$this->form_validation->set_rules('sub_category', 'Sub Category', 'required|numeric', array(
						'required' => 'Sub Category is required',
					));
					$this->form_validation->set_rules('video', 'Video', 'required', array(
					...
					if ($this->form_validation->run() == FALSE) {
						...
						$checkCategory = $this->dynamic_model->getdatafromtable('business_category', array(
						...
						if (!$checkCategory) {
							...
							$checkSubCategory = $this->dynamic_model->getdatafromtable('business_category', array(
							...
							if (!$checkSubCategory) {
								...
								$implode = implode(",", $video);
								$checkWeekDays = $this->db->query('SELECT * FROM `manage_videos` WHERE `id` IN (' . $implode . ')');
								if ($checkWeekDays->num_rows() === count($video)) {
									$arg['status']    = 1;
									$arg['error_code'] = HTTP_OK;
									$arg['error_line'] = __line__;
									$arg['data']       = $checkWeekDays->result_array();
									$arg['message']   = $this->lang->line('record_found');
								} else {
									$arg['status']     = 0;
									$arg['error_code']  = REST_Controller::HTTP_NOT_FOUND;
